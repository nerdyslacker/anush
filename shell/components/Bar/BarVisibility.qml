...
Singleton {
    ...
    readonly property var defaultClusters: ({
            ...
            right: ["media", "weather", "metrics", "battery", "brightness", "volume", "micIndicator", "network", "bluetooth", "keyboard", "clipboard", "tray", "notifications", "clock", "capsLock", "screenshot", "commands"]
        })
    ...
    function setBarPosition(position) {
        const next = ["top", "bottom", "left", "right"].indexOf(position) >= 0 ? position : "top";
        if (barPosition === next)
            return;
        barPosition = next;
    ...
    function writeState(widgetState, showAll, clusterState) {
        ...
        for (const name in defaults) {
            ...
            savedWidgets[name] = (info && info.mandatory === true) || widgetState[name] !== false;
        }
        ShellState.updateSection("bar", {
    ...
    function loadState() {
        const saved = ShellState.state.bar;
        const next = ({});
        ...
        root.barPosition = ["top", "bottom", "left", "right"].indexOf(saved.position) >= 0 ? saved.position : "top";
