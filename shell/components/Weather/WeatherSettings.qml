...
Popout {
    ...
    function saveSettings() {
        ...
        ShellState.updateSection("weather", {
            location: location,
            units: unitsDraft
        })
        visible = false
    ...
    function loadState() {
        root.savedLocation = String(ShellState.state.weather.location ?? "")
        const value = String(ShellState.state.weather.units ?? "").toLowerCase()
        root.savedUnits = value === "c" || value === "f" ? value : ""
        if (!visible)
            root.unitsDraft = root.savedUnits
    }
