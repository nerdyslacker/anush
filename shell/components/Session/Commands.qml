...
BarModule {
    ...
    function persistPomo() {
        ShellState.updateSection("pomodoro", {
            endMs: Math.round(pomoEndMs), minutes: pomoMinutes
        })
    }
    ...
    function loadPomodoro() {
        const saved = ShellState.state.pomodoro
        const end = Number(saved.endMs) || 0
        const minutes = Number(saved.minutes) || 0
    ...
    Connections {
        target: ShellState
        function onStateChanged() { root.loadPomodoro() }
        function onReadyChanged() { if (ShellState.ready) root.loadPomodoro() }
    }

    Component.onCompleted: if (ShellState.ready) loadPomodoro()
